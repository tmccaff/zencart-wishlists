...
<?php if ( isset($records) && $records->RecordCount() > 1 ) { ?>
			...
			<?php while (!$records->EOF) { ?>
				<?php $rows++; ?>
				<dt><?php echo $rows; ?>. <a href="<?php echo zen_href_link(FILENAME_WISHLIST_FIND, 'wid='.$records->fields['id'], 'SSL'); ?>"><?php echo get_fullname($records->fields['customers_firstname'], $records->fields['customers_lastname'], $records->fields['customers_email_address']) . FIND_RESULT_WISHLIST_OWNER; ?></a></dt>
				<dd>
				<ul>
				<?php $location = un_get_citystate($records->fields['entry_city'], $records->fields['entry_state'], 'Unknown'); ?>
				<?php if ( !is_empty($location) ) { ?>
				<li><?php echo FIND_RESULT_LOCATION . LABEL_DELIMITER; ?>&nbsp;<?php echo $location; ?></li>
				<?php } ?>
				<?php if ( !is_empty($records->fields['name']) ) { ?>
				<li><?php echo FIND_RESULT_LIST_NAME . LABEL_DELIMITER; ?>&nbsp;<?php echo $records->fields['name']; ?></li>
				<?php } ?>
				<?php if ( !is_empty($records->fields['comment']) ) { ?>
